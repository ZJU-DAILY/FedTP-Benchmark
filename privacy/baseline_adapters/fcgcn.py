...
class FCGCNPrivacyAdapter:
    ...
    def build_model(self, args: Any, batch: Any) -> nn.Module:
        ...
        adj = _fcgcn_adj_from_batch(args, batch, num_nodes)
        model = FCGCN(
            num_nodes=num_nodes,
            in_dim=args.t_in,
            out_dim=args.t_out,
    ...
    def compute_leak(
        ...
        if leak_type != "gradient":
            raise ValueError("FCGCN currently supports gradient and model_update reconstruction only.")
...
def _normalize_fcgcn_adj(adj: torch.Tensor) -> torch.Tensor:
    adj = adj.clone()
...
def _edge_index_fallback_adj(batch: Any, num_nodes: int, device: torch.device | str) -> torch.Tensor:
    ...
    if edge_index is not None:
        ...
        if edge_index.dim() == 2 and edge_index.shape[0] == 2 and edge_index.numel() > 0:
            ...
            if row.numel() > 0:
                adj[row, col] = 1.0
                adj[col, row] = 1.0
    return _normalize_fcgcn_adj(adj)
...
def _read_distance_rows(distance_path: Path) -> list[dict[str, float]]:
    with distance_path.open("r", newline="", encoding="utf-8-sig") as f:
        ...
        if reader.fieldnames is None:
            raise ValueError(f"[FCGCN] distance.csv is empty: {distance_path}")
        required = {"from", "to", "cost"}
        if not required.issubset(set(reader.fieldnames)):
            raise ValueError(
                f"[FCGCN] distance.csv must contain from/to/cost columns, got {reader.fieldnames}"
            )
...
def _fcgcn_adj_from_batch(args: Any, batch: Any, num_nodes: int) -> torch.Tensor:
    device = args.device
    dataset_name = "PeMSD7" if args.dataset_name == "PeMS07" else args.dataset_name
    distance_path = _repo_root() / "data" / dataset_name / "distance.csv"

    if not distance_path.exists():
        return _edge_index_fallback_adj(batch, num_nodes, device)
    ...
    a_hat = _normalize_fcgcn_adj(local_adj)

    if dataset_name == "PeMSD7":
        # Keep the privacy model aligned with run/fate_main.py's FCGCN-D7 branch.
        eye = torch.eye(num_nodes, dtype=a_hat.dtype, device=a_hat.device)
        a_hat = 0.5 * (eye + eye.T)
