...
class GRU(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, pre_len:int, **kwargs):
        super(GRU, self).__init__()
        self.pre_len=pre_len
        self._input_dim = input_dim  # num_nodes for prediction
        self._hidden_dim = hidden_dim
        self.gru_cell = GRUCell(self._input_dim, self._hidden_dim)
        self.linear=nn.Linear(self._hidden_dim,self.pre_len)
    ...
    def forward(self, inputs):
        ...
        output=self.linear(last_output)#torch.Size([32, 26, 1])
